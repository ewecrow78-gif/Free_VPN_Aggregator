...
| **Топ 50 быстрых** | `{top_50_fast}` | |
| **Топ 30 быстрых** | `{top_30_fast}` | |
| **Все конфиги** | `{all_configs}` | Без лимитов |
| **Белые списки 100** | `{wl_100}` | |
| **Топ 50 белых списков** | `{wl_50}` | |
| **Топ 30 белых списков** | `{wl_30}` | |
| **Все вместе** | `{mixed_100}` | 50 обычных + 50 белых |
| **Все белые списки** | `{all_wl}` | Без лимитов |
| **Топ быстрых 100** | `{fastest_all_100}` | Обычные и белые |
...
> Рекомендуемые: **[top_50_fast]({base_url}base64/top_50_fast.txt)**, **[wl_100]({base_url}base64/wl_100.txt)**, **[mixed_100]({base_url}base64/mixed_100.txt)**.
...
def build_readme():
    ...
    cards = {
        "card_top_50_fast": sub_card(
            "🚀 ТОП 50 БЫСТРЫХ",
            "top_50_fast",
            "50 лучших обычных конфигов (не белые списки) с самым низким пингом.",
            badge="⚫",
            count_hint=str(stats.get("top_50_fast", "~")),
            raw_rel="top_50_fast.txt",
        ),
        "card_top_30_fast": sub_card(
            "🚀 ТОП 30 БЫСТРЫХ",
            "top_30_fast",
            "30 лучших обычных конфигов (не белые списки) с самым низким пингом.",
            badge="⚫",
            count_hint=str(stats.get("top_30_fast", "~")),
            raw_rel="top_30_fast.txt",
        ),
        "card_all_configs": sub_card(
            "📚 ВСЕ КОНФИГИ (Без лимитов)",
            "all_configs",
            "Абсолютно все живые конфигурации в одном месте.",
            badge="⚫",
            count_hint=str(stats.get("all_configs", "~")),
            raw_rel="all_configs.txt",
        ),
        "card_wl_100": sub_card(
            "⚪ БЕЛЫЕ СПИСКИ (До 100)",
            "wl_100",
            "Обход блокировок по IP/SNI. Скомпилировано максимум 100 штук.",
            badge="⚪",
            count_hint=str(stats.get("wl_100", "~")),
            raw_rel="whitelists/wl_100.txt",
        ),
        "card_wl_50": sub_card(
            "⚪ ТОП 50 БЕЛЫХ СПИСКОВ",
            "wl_50",
            "Топ 50 лучших конфигов для обхода жестких белых списков (DPI).",
            badge="⚪",
            count_hint=str(stats.get("wl_50", "~")),
            raw_rel="whitelists/wl_50.txt",
        ),
        "card_wl_30": sub_card(
            "⚪ ТОП 30 БЕЛЫХ СПИСКОВ",
            "wl_30",
            "Топ 30 лучших конфигов для обхода жестких белых списков (DPI).",
            badge="⚪",
            count_hint=str(stats.get("wl_30", "~")),
            raw_rel="whitelists/wl_30.txt",
        ),
        "card_mixed_100": sub_card(
            "🔄 ВСЕ ВМЕСТЕ (50/50)",
            "mixed_100",
            "Микс: 50 быстрых обычных конфигураций + 50 для белых списков.",
            badge="☯️",
            count_hint=str(stats.get("mixed_100", "~")),
            raw_rel="mixed_100.txt",
        ),
        "card_all_wl": sub_card(
            "⚪ ВСЕ БЕЛЫЕ СПИСКИ (Без лимитов)",
            "all_wl",
            "Полный набор всех конфигов, проходящих проверку на белые списки.",
            badge="⚪",
            count_hint=str(stats.get("all_wl", "~")),
            raw_rel="whitelists/all_wl.txt",
        ),
        "card_fastest_all_100": sub_card(
            "⚡ ТОП САМЫХ БЫСТРЫХ (Любые)",
            "fastest_all_100",
            "Топ 100 конфигов с самым низким пингом, без разницы простые они или белые списки.",
            badge="⚡",
            count_hint=str(stats.get("fastest_all_100", "~")),
            raw_rel="fastest_all_100.txt",
        ),
    }
    ...
    readme_content = README_TEMPLATE.format(
        ...
        top_50_fast=stats.get("top_50_fast", 0),
        top_30_fast=stats.get("top_30_fast", 0),
        all_configs=stats.get("all_configs", 0),
        wl_100=stats.get("wl_100", 0),
        wl_50=stats.get("wl_50", 0),
        wl_30=stats.get("wl_30", 0),
        mixed_100=stats.get("mixed_100", 0),
        all_wl=stats.get("all_wl", 0),
        fastest_all_100=stats.get("fastest_all_100", 0),
